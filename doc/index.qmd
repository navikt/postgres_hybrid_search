...
## Changelog
- **31.01.25**: Endret `rank` til `rank`
    - Det var en bug i den originale versjonen, kopiert fra supabase, hvor koden
    brukte `rank` for å ha en ID på rangen fra et undersøk. Dette kan gi
    et skjevt bilde hvis flere treff skulle hatt den samme rangen fra et
    undersøk. Ved å benytte `rank` får alle like treff den samme ID-en.
- **26.08.24**: Utgitt
:::
