...
søkene. Hvert søk gir tekstene som returneres en rangering ($rangering_i$), i
[ligning @eq-rang] tar vi hensyn til at et treff ikke nødvendigvis finnes i et
søk, og vi summerer deretter de forskjellige rangeringene til én endelig
sum ($score$) i [ligning @eq-rrf]. Denne summen kan så sorteres for å fine
endelig rangering.
...
forskjellige søk. For å finne den endelige rangeringen summerer vi disse
resultatene med $\frac{1}{5} + \frac{1}{4} = 0.2 + 0.25 = 0.45$.

Hvis vi har en annen tekst i det samme hybridsøket som fikk rangering $3$ fra
det ene søket og ikke ble funnet i det andre søket får vi en _RRF_ sum
...
_RRF_ sum på $0.45$, men nå sier vi at for den andre teksten har den fått
rangering $1$ for et søk og ikke til stedet i det andre søket. Tekst to har
derfor en oppdatert _RRF_ sum på $\frac{1}{1} + 0 = 1$ og vil derfor være
rangert høyere enn den første teksten bare fordi den er høyt oppe i et enkelt
søk.

Siden vi ønsker at en tekst som er tilstede i kun et søk skal jevnes ut bruker
vi en utjevningskonstant $k=60$. For den første teksten får vi en ny _RRF_ sum
på $\frac{1}{60 + 5} + \frac{1}{60 + 4} ≈ 0.031$, mens for den andre teksten får
vi en _RRF_ sum på $\frac{1}{60 + 1} ≈ 0.016$. Vi ser at med
utjevningskonstanten vår vil den første teksten fortsatt være rangert over den
andre teksten og konstanten hadde ønsket effekt.
:::
...
`text-embedding-3-large` med en størrelses reduksjon til $2000$ burde dette
oppsettet fungere godt.
...
`vector_cosine_ops` og `<=>` (cosine similarity) operatorene i `pgvector` og få
